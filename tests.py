...
class AVLTree:
    ...
    def right_rotate(self, root: Node) -> Optional[Node]:
        """
        This function performs a right rotation on the subtree rooted at root.
        Returns: root of new subtree after rotation.
        """
        if root is None:
            return None

        # pull left child up and shift right of left child across tree, update parent
    ...
    def balance_factor(self, root: Node) -> int:
        """
        Computes the balance factor for a given root
        Returns: integer representing the balance factor of root.
        """
        if root is None:
            return 0
    ...
    def rebalance(self, root: Node) -> Optional[Node]:
        """
            Rebalances the AVL tree at a given node and rotates if needed
            Returns: None only updates the tree
        """
        if root is None:
            return None
        ...
        if balance_factor >= 2:  # left heavy
            if self.height(root.left.left) >= self.height(root.left.right):
                root = self.right_rotate(root)  # left-left case
            else:
                root.left = self.left_rotate(root.left)  # left-right case
                root = self.right_rotate(root)  # left-left case
        elif balance_factor <= -2:  # right heavy
            if self.height(root.right.right) >= self.height(root.right.left):
                root = self.left_rotate(root)  # right-right case
            else:
                root.right = self.right_rotate(root.right)  # right-left case
                root = self.left_rotate(root)  # right-right case
    ...
    def get_height(self, node: Node) -> int:
        """
        Get the height of a given node in the AVL Tree.
    ...
    def insert(self, root: Node, val: T, data: List[str] = None) -> Optional[Node]:
        """
        This function insers a new node into the given Key/Value then it rebalances and fixes the tree if neccessary
        Returns: None but will return the root of the subtree if rembalancing is needed
        """
        ...
        # perform rotations if balance factor is greater than 1 or less than -1
    ...
    def min(self, root: Node) -> Optional[Node]:
        """
        This function finds the node with the smaller value in the subtree
        Returns: Node that is smaller
        """
        if root is None:
            return None
    ...
    def max(self, root: Node) -> Optional[Node]:
        """
        This function finds the node with the largest value in the subtree
        Returns: Node that is larger
        """
        if root is None:
            return None
    ...
    def search(self, root: Node, val: T) -> Optional[Node]:
        """
        This function finds the needed node in the AVL tree given its value
        Returns:
        - If a Node containing the value val is found, return that Node.
        - If a Node containing the value val is not found, return the parent Node where val would be inserted.
    ...
    def __iter__(self) -> Generator[Node, None, None]:
        """
        Iterates the nodes using IN-order traversal
        Returns: Each node that it iterates over
        """
    ...
    def postorder(self, root: Node) -> Generator[Node, None, None]:
        """
        Traverses the tree using Post Order
        Returns: each node in order
        """
        if root is None:
            return
    ...
    def levelorder(self, root: Node) -> Generator[Node, None, None]:
        """
        Traverses the tree using Level Order
        Returns: each node in order
        """
        if not root:
            return
...
class Table:
    ...
    def is_empty(self):
        return not bool(self.data_tree)
    ...
    def insert_rows(self, values: List[List[str]], attributes: List[str]) -> None:
        ...
        for row_values in values:
            row_index = self.insertion_index
            row_data = [row_values[self.names_to_indices[attr]] for attr in attributes]
            self.data_tree.insert(row_index, row_data)
    ...
    def remove_rows(self, indices: List[str]) -> None:
        ...
        for index in map(int, indices):
            self.data_tree.remove(index)

    def iter_helper(self, node: Node, reverse=False) -> Generator:
        if node is None:
            node = self.data_tree.root
        stack = []
        while stack or node:
            while node:
                stack.append(node)
    ...
    def show_latest(self) -> List[List[str]]:
        ...
        latest_node = next(self.iter_helper(self.data_tree.root, reverse=True))
        latest_index, latest_data = latest_node.key, latest_node.value
        latest_values = [str(latest_index)] + latest_data
        return [self.header_row, latest_values]
    ...
    def show_oldest(self) -> List[List[str]]:
            ...
            if self.is_empty():
                return [self.header_row]
            oldest_node = next(self.iter_helper())
            oldest_index, oldest_data = oldest_node.key, oldest_node.value
            oldest_values = [str(oldest_index)] + oldest_data
            return [self.header_row, oldest_values]
    ...
    def show_everything(self) -> List[List[str]]:
                ...
                for index, data in self.data_tree:
                    row_values = [str(index)] + data
